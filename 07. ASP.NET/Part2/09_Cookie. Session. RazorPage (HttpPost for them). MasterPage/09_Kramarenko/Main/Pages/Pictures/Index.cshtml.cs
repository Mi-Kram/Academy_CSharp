...
namespace Main.Pages.Pictures
{
    public class IndexModel : PageModel
    {
		...
		public async Task<JsonResult> OnPostByTypeAsync(int? typeID)
		{
			...
			else if (typeID == 0) Pictures = await _context.Pictures.Include(x => x.Category).ToListAsync();
			else if (typeID == -1 && User.Identity.IsAuthenticated)
			{
				string myID = await GetMyIDAsync();
				Pictures = await _context.Pictures.Where(x => x.UserID == myID).Include(x => x.Category).ToListAsync();
			}
			else Pictures = await _context.Pictures.Where(x => x.CategoryID == typeID).Include(x => x.Category).ToListAsync();
